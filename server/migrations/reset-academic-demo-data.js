...
const createModule = async ({ module_name, department }) => {
  return insertAndReturnId(
    ...
    [
      ...
      department === 'Networking' ? 'Networking Lab' : 'Computer Lab'
    ]
  );
};
...
const run = async () => {
  ...
  for (const moduleName of softwareModules) {
    moduleIds[moduleName] = await createModule({ module_name: moduleName, department: 'Software Development' });
  }
  for (const moduleName of networkingModules) {
    moduleIds[moduleName] = await createModule({ module_name: moduleName, department: 'Networking' });
  }
  ...
  for (const classRecord of classRecords) {
    for (const day of FIXED_DAYS) {
      ...
      for (const period of periods) {
        const assignment = classRecord.assignments[(period.slot_number - 1) % classRecord.assignments.length];
        await pool.execute(
